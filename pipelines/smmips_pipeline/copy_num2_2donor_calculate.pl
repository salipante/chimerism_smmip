...
#calculate the donor1 geometric mean
...
#calculate the donor2 geometric mean
while(my $sample_line = <MAIN2>) {
   my @sample_split = split("\t",$sample_line);
   if(($sample_split[0] eq "46D_cont_smg1_277") || ($sample_split[0] eq "47C_cont_dnmt1_328")) { #skip two bad controls
      next;
   }
   if($sample_split[51] >= $min_reads) {
      push @main2, $sample_split[51];
      $geomean_total=$geomean_total*$sample_split[51];
      $non_zero_count++;
   }
   else {
...
my $main2_geomean = $geomean_total ** (1/$non_zero_count);
$non_zero_count=0;
$geomean_total=1;
...
foreach my $count (@main2) {
   push @main2_div_geo, $count/$main2_geomean;
}
...
foreach my $count (85,86,87,88,89,90,91,92,93,94,95,170,171,172) {
   $major_minor_average_factor=$major_minor_average_factor+($main_div_geo[$count]+$main2_div_geo[$count])/$minor_div_geo[$count];
...
for(my $i=0;$i < 173;$i++) {
   my $total= ($main_div_geo[$i]+$main2_div_geo[$i])*((100-$spike)/100)+$minor_expected_value[$i]*($spike/100);
#   print "main $main[$i] - $main_div_geo[$i] * (100-$spike)/100 + $minor_expected_value[$i] * ($spike/100) total is $total\n";
   push @model, ($main_div_geo[$i]+$main2_div_geo[$i])*((100-$spike)/100)+$minor_expected_value[$i]*($spike/100);
}
...
for(my $i=0;$i < 173;$i++) {
   ...
   if(($main[$i] == 0) && ($minor[$i] == 0) && ($main2[$i] == 0)) { # || ($i ~~ [85,86,87,88,89,90,91,92,93,94,95,170,171,172])) {
      print SKIP "$mip_name[$i]\t$main[$i]\t$main_div_geo[$i]\t$minor[$i]\t$minor_div_geo[$i]\t$minor_expected_value[$i]\t$model[$i]\t$combo[$i]\t$combo_div_geo[$i]\t$final_value[$i]\t$final_answer[$i]\n";
   }
   elsif (($main[$i] == 0) && ($main2[$i] == 0)) {
